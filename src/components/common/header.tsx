  ...
  return (
    <header {...props} className={cm('sticky top-0 z-50', className)}>
      <div className='flex h-[56px] items-center gap-3.5 bg-white pl-4 pr-2'>
        <Link href='/'>
          <Image src='/assets/icons/common/logo-title.svg' alt='logo' width={18} height={32} />
        </Link>
        <Link
          href='/search'
          className='flex h-10 flex-1 items-center gap-2 rounded-md bg-grey-90 px-3'
        >
          <Image src='/assets/icons/common/search.svg' alt='logo' width={24} height={24} />
        ...
        <Link href='/'>
          <Image src='/assets/icons/common/cart-title.svg' alt='logo' width={22} height={23} />
        </Link>
      </div>
    </header>
  );
